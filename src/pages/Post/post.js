...
export default function Post() {
  ...
  const renderPostContent = () => {
    ...
    const extractAppleMusicLinks = (content) => {
      const regex = /https:\/\/music\.apple\.com\/[^\s"]+/g;
      const matches = content.match(regex);
      return matches ? matches : [];
    };        
    ...
    const replaceAppleMusicLinks = (content) => {
      console.log("replaceAppleMusicLinks is running")
      const regex = /https:\/\/music\.apple\.com\/[^\s]+/g;
      return content.replace(regex, (match) => {
        console.log("APPLEMUSIC")
        const embedUrl = match.replace('https://music.apple.com', ''); // Remove the base URL
        console.log(`<iframe style="width: 70%" allow="autoplay *; encrypted-media *; fullscreen *; clipboard-write" frameBorder="0" height="175" sandbox="allow-forms allow-popups allow-same-origin allow-scripts allow-storage-access-by-user-activation allow-top-navigation-by-user-activation" src="https://embed.music.apple.com${embedUrl}"></iframe>`)
        return `<iframe style="width: 70%" allow="autoplay *; encrypted-media *; fullscreen *; clipboard-write" frameBorder="0" height="450" sandbox="allow-forms allow-popups allow-same-origin allow-scripts allow-storage-access-by-user-activation allow-top-navigation-by-user-activation" src="https://embed.music.apple.com${embedUrl}"></iframe>`;
      });
    };
    
    // Function to extract the tweet ID from the tweet link
    const extractTwitterEmbed = (content) => {

      const regex = /(&lt;blockquote class="twitter-tweet"&gt;[\s\S]*?&lt;\/script&gt;)/g;
      const matches = content.match(regex);
      return matches ? matches : [];
    };

    // Get the tweet links from the post description
    const tweetEmbeds = extractTwitterEmbed(htmlContent);
    console.log(tweetEmbeds)
    ...
    console.log(appleMusicLinks)

    let replacedContent = htmlContent;
    tweetEmbeds.forEach((embed) => {
      replacedContent = replacedContent.replace(embed, replaceTweetLinks(embed));
    });
